...
int gerarAssembly(const std::vector<std::string> &_tokens_, std::string &codigoAssembly)
{
    ...
    for (const auto &token_str : _tokens_)
    {
        std::pair<std::string, std::string> token = splitToken(token_str);
        std::string tipo = token.first;
        std::string valor = token.second;

        // Se for numero salva como literal (constante)
        if (tipo == T_NUMERO)
        {
            if (literais.find(valor) == literais.end())
            {
                literais[valor] = "LIT_" + std::to_string(contagemLiterais++);
            }
        }
        // Se for letra maiuscula salva como variavel
        else if (tipo == T_IDENTIFICADOR)
        {
            if (variaveis.find(valor) == variaveis.end())
            {
                variaveis[valor] = "VAR_" + valor;
            }
        }
    }
    ...
    {
        auto token = splitToken(_tokens_[i]);
        std::string tipo = token.first;
        std::string valor = token.second;
        ...
        else if (tipo == T_NUMERO)
        {
            ...
            ss << "    @ Empilha Constante: " << valor << "\n";
            ss << "    LDR R0, =" << literais[valor] << "\n";
            ss << "    VLDR.F64 D0, [R0]\n";
            ss << "    VPUSH.F64 {D0}\n\n";
        }
        else if (tipo == T_IDENTIFICADOR)
        {
            ...
            if (i > 0)
            {
                auto prev_token = splitToken(_tokens_[i - 1]);
                if (prev_token.first == T_PAREN_ESQ)
                {
                    simulated_stack++;
                    ss << "    @ LOAD: Le a variavel " << valor << " da memoria e empilha\n";
                    ss << "    LDR R0, =" << variaveis[valor] << "\n";
                    ss << "    VLDR.F64 D0, [R0]\n";
                    ss << "    VPUSH.F64 {D0}\n\n";
                }
            }
            ...
            else
            {
                ...
                ss << "    @ Grava resultado na variavel " << valor << "\n";
                ss << "    VPOP.F64 {D0}\n";
                ss << "    LDR R0, =" << variaveis[valor] << "\n";
                ss << "    VSTR.F64 D0, [R0]\n\n";
            }
